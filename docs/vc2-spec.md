...
**合并版**：项目宪法 v1.1 + 生产纪律 v1.6 + 部署经验 v2.1 + 前端规范 v3.0
**生效日期**：2026-05-28
**版本**：v3.0
**修订说明**：新增前端规范章节（第八章修订），新增偏离记录（第十八章），各章补充可执行检查项。
...
### 第三章检查项
- [ ] Pipeline 输出包含全部 7 个强制字段
- [ ] 字段类型符合预期（string, number）
...
### 第八章检查项
- [ ] 前端使用 Tailwind CSS 原子类，不使用 Shadcn UI
- [ ] 前端图标统一使用 lucide-react
- [ ] 后端使用 Python FastAPI，不使用 TypeScript Next.js API Routes
- [ ] API 服务端口 8000，路由前缀 `/api/v1/`
...
| search.ibotclaw.com | AI采购助手 | → vc2_search:8001 |

**HTTPS**：全站强制HTTPS，证书存于 `docker/ssl-backup/`。

### 第九章检查项
- [ ] 域名 DNS 解析指向正确 IP
- [ ] Nginx 配置包含 HTTPS 重定向
- [ ] 证书文件存在且未过期
...
> 本章记录 v3.0 与 v2.1 之间因实际实现导致的规范偏离。

| 偏离编号 | 章节 | 规范原定 | 实际偏离 | 偏离原因 | 记录日期 |
|----------|------|----------|----------|----------|----------|
| D-20260528-001 | 第八章 技术选型 | 后端：TypeScript Next.js API Routes | 后端：Python FastAPI | 早期规划与实际实现不符，Python FastAPI 更适合数据处理场景 | 2026-05-28 |
| D-20260528-002 | 第八章 技术选型 | 前端：Shadcn UI | 前端：Tailwind + lucide-react | 为保持轻量，未引入组件库，直接使用 Tailwind 原子类 | 2026-05-28 |

### 偏离处理原则
1. 偏离记录仅作为事实存档，不作为变更依据
2. 如需正式修订规范，按第十八章程序执行
3. 新增资产或重大变更时，优先遵循现行规范
...
| 编号 | 类型 | 位置 | 变更内容 |
|------|------|------|---------|
| 1 | 修订 | 第八章 | TypeScript→Python FastAPI, Shadcn UI→Tailwind+lucide-react |
| 2 | 新增 | 各章 | 补充可执行检查项（第一章至第十一章） |
| 3 | 新增 | 第十八章 | 偏离记录章节（D-20260528-001, D-20260528-002） |
| 4 | 新增 | .claude/rules/frontend.md | 前端规范 v3.0（认证/API/组件/Middleware/路由/错误处理） |
